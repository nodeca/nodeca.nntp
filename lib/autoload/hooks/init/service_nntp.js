...
module.exports = function (N) {
  ...
  N.wire.on('init:services.nntp', async function server_bind(N) {
    let bindings = extractBindings(_.get(N.config, 'nntp', {}));
    ...
    async function bindServer(name) {
      ...
      if (ssl) {
        for (let val of [ 'key', 'cert' ]) {
          ...
          try {
            ssl[val] = await fs.readFile(filename, 'utf8');
          } catch (err) {
      ...
      async function reload_cert() {
        ...
        for (let val of [ 'key', 'cert' ]) {
          try {
            creds[val] = await fs.readFile(files[val], 'utf8');
          } catch (err) {
        ...
        // Using undocumented API to change SSL certificates on the fly:
        // https://github.com/nodejs/node/issues/4464#issuecomment-296004502
        //
        // TODO: switch to official API when node.js implements it
        //
        nntp.server._sharedCreds.context.setCert(creds.cert);
        nntp.server._sharedCreds.context.setKey(creds.key);
      }

      if (ssl) {
        fs.watchFile(files.key,  { persistent: false, interval: 60 * 60 * 1000 }, reload_cert);
        fs.watchFile(files.cert, { persistent: false, interval: 60 * 60 * 1000 }, reload_cert);
      }
