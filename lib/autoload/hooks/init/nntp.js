...
module.exports = function (N) {
  ...
  N.wire.before('init:server.nntp', function init_nntp_methods(nntp) {
    ...
    nntp._buildBody = function (session, article) {
      let locale = session.user_info && session.user_info.locale || N.config.locales[0];
      let helpers = {};
      ...
      return new Buffer(html).toString('base64').match(/.{1,76}/g);
    };
    ...
    nntp._getUserInfo = async function (session) {
      ...
      if (!session.user_info) session.user_info = await userInfo(N, null);
